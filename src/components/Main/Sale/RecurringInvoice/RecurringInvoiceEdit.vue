        ...
        <v-layout>
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>
              Recurring Invoice - {{this.editedItem.SaleRecurringInvoiceId}} - {{this.editedItem.StatusName}}
              <v-toolbar-items>
                ...
                <v-btn flat color="colorLight" to="/RecurringInvoice">close</v-btn>
              </v-toolbar-items>
            </v-card-title>
            <v-card>
              <v-layout>
              ...
              <v-layout>
                <v-flex lg2 class="pa-1">
                  <auto-complete
                    ...
                    :isAsync="true"
                    :apiUrl="'SaleRecurringInvoice/GetCustomer?pSearch='"
...
export default {
  ...
  methods: {
    onChildClick(obj) {
      if (obj != null) {
        this.editedItem.SaleRecurringInvoiceDetail = obj.tableRow;
    ...
    async tableLoadDetail(id) {
      let response = await axios.get(
        this.$urlApplication + "SaleRecurringInvoice/" + id
      );
      if (id == 0) {
      ...
      this.listOfRecordDetails = data.SaleRecurringInvoiceDetailModel;
      this.editedItem.SaleRecurringInvoiceDetail =
        data.SaleRecurringInvoiceDetailModel;
    ...
    post() {
      this.editedItem.SettingFrequencyId = this.editedItem.selectSettingFrequency.SettingFrequencyId;
      this.editedItem.SettingStatusId = this.editedItem.selectSettingStatus.SettingStatusId;
      this.editedItem.StartDate = this.StartDate;
      ...
      axios({
        method: "post",
        url: this.$urlApplication + "SaleRecurringInvoice/Post",
        data: this.editedItem
      })
        .then(res => {
          this.$router.push("/RecurringInvoice");
    ...
    put() {
      ...
      axios({
        ...
        url:
          this.$urlApplication +
          "SaleRecurringInvoice/Put?id=" +
          this.editedItem.SaleRecurringInvoiceId,
        data: this.editedItem
      })
        .then(res => {
          this.$router.push("/RecurringInvoice");
    ...
    async getProposedNumber() {
      const res = await axios
        .get(this.$urlApplication + "SaleRecurringInvoice/GetProposedNumber")
        .then(res => {

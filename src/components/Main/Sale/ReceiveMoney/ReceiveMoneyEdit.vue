        ...
        <v-layout>
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>
              Receive Money - {{this.editedItem.SaleRecurringInvoiceId}} - {{this.editedItem.StatusName}}
              <v-toolbar-items>
                ...
                <v-btn flat color="colorLight" to="/SaleReceiveMoney">close</v-btn>
              </v-toolbar-items>
            </v-card-title>
            <v-card>
              <v-layout>
                <v-flex lg4 class="pa-1">
                  <auto-complete
                    ...
                    :isAsync="true"
                    :apiUrl="'SaleReceiveMoney/GetCustomer?pSearch='"
                ...
                <v-flex lg3 class="pa-1">
                  <v-text-field
                    ...
                    :append-icon="'today'"
                    type="date"
                    :value="ReceiveMoneyDate && ReceiveMoneyDate.toISOString().split('T')[0]"
                  ></v-text-field>
                </v-flex>
                <v-flex lg2 class="pa-1">
                  <v-text-field
...
export default {
  ...
  methods: {
    onChildClick(obj) {
      if (obj != null) {
        this.editedItem.SaleReceiveMoneyDetail = obj.tableRow;
    ...
    async tableLoadDetail(id) {
      let response = await axios.get(
        this.$urlApplication + "SaleReceiveMoney/" + id
      );
      if (id == 0) {
    ...
    post() {
      ...
      this.editedItem.ReceiveMoneyDate = this.ReceiveMoneyDate;
      this.IsSnackBar = true;

      axios({
        method: "post",
        url: this.$urlApplication + "SaleReceiveMoney/Post",
        data: this.editedItem
      })
        .then(res => {
          this.$router.push("/SaleReceiveMoney");
    ...
    clear() {
      ...
      this.editedItem.ReceiveMoneyDate = new Date();
    ...
    async getProposedNumber() {
      const res = await axios
        .get(this.$urlApplication + "SaleReceiveMoney/GetProposedNumber")
        .then(res => {

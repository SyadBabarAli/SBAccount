  ...
  <v-layout row wrap class="pa-2">
    <v-flex lg12 class="mt-12">
      <v-card class="black--text elevation-5">
        <v-layout>
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>Sale Return</v-card-title>
            <v-card-title>
              ...
              <v-text-field
                ...
                hide-details
              ></v-text-field>
              <v-btn slot="activator" color="primary" dark class="mb-2" to="/SaleReturnEdit?id=0">Add</v-btn>
            </v-card-title>
            <v-layout lg12>
                ...
                <table class="csmTable">
                  <thead>
                    <tr>
                      ...
                      <th v-on:click="sortTable('SaleReturnDate')">Date</th>
                  ...
                  <tbody>
                    <tr v-for="row in rows">
                      ...
                      <td class="text-xs-left">{{row.SaleReturnDate | momentConvertDate }}</td>
                      ...
                      <td class="justify-center layout px-0">
                        <v-btn
                          ...
                          :to="'/SaleReturnEdit?id=' +row.SaleReturnId"
                        >
...
export default {
  ...
  methods: {
    ...
    async tableLoad() {
      ...
      axios({
        method: "put",
        url: this.$urlApplication + "SaleReturn/GetSaleReturn",
        data: pagedResult
      })
        .then(res => {
    ...
    delete(id) {
      ...
      axios({
        method: "delete",
        url: this.$urlApplication + "SaleReturn/Delete?companyId=1&id=" + pId
      })
        .then(res => {

  ...
  <v-layout row wrap class="pa-2">
    <v-flex lg12 class="mt-12">
      <v-card class="black--text elevation-5">
        <v-layout>
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>Category Brand</v-card-title>
            <v-card-title>
            ...
            >
              <template slot="items" slot-scope="props">
                <td class="text-xs-left">{{ props.item.CategoryBrandId }}</td>
...
export default {
  ...
  data: () => ({
    ...
    search: "",
    headers: [
      { text: "CategoryBrandId", value: "CategoryBrandId" },
  ...
  methods: {
    ...
    async tableLoad() {
      ...
      const res = await axios
        .get(this.$urlApplication + "CategoryBrand/GetCategoryBrands")
        .then(res => {
    ...
    post() {
      ...
      axios({
        method: "post",
        url: this.$urlApplication + "CategoryBrand/Post",
        data: obj
      })
        .then(res => {
    ...
    delete(id) {
      ...
      axios({
        method: "delete",
        url:
          this.$urlApplication + "CategoryBrand/Delete?companyId=1&id=" + pId
      })
        .then(res => {
          this.IsSnackBar = false;
          this.tableLoad();
    ...
    clear() {
      this.editedItem.CategoryBrandName = "";
    ...
    dialogOpen() {
      if (this.editedItem.CategoryBrandId == undefined) {
        ...
        //Edite Record
        //this.changeCategoryBrands(this.selectCategoryBrands);
      }
    },
    close() {
      if (!this.dialog) {

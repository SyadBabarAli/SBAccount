  ...
  <v-layout row wrap class="pa-2">
    <v-flex lg12 class="mt-12">
      <v-card class="black--text elevation-5">
        <v-layout>
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>Category Product</v-card-title>
            <v-card-title>
                    ...
                    <v-container grid-list-md>
                      <v-card class="elevation-10">
                        <v-card-title
                          ...
                          primary-title
                        >Category Product</v-card-title>
                        <v-divider></v-divider>
                        <!-- <v-flex lg6>
                          <v-text-field
                            v-model="editedItem.CategoryProductId"
                            label="CategoryProductId"
                          ></v-text-field>
                        </v-flex>-->
            ...
            >
              <template slot="items" slot-scope="props">
                <td class="text-xs-left">{{ props.item.CategoryProductId }}</td>
...
export default {
  ...
  data: () => ({
    ...
    search: "",
    headers: [
      { text: "CategoryProductId", value: "CategoryProductId" },
  ...
  methods: {
    ...
    async tableLoad() {
      ...
      const res = await axios
        .get(this.$urlApplication + "CategoryProduct/GetCategoryProducts")
        .then(res => {
    ...
    post() {
      ...
      axios({
        method: "post",
        url: this.$urlApplication + "CategoryProduct/Post",
        data: obj
      })
        .then(res => {
    ...
    put() {
      var obj = this.editedItem;
      obj.CompanyId = 1;
      obj.CategoryProductId = this.editedItem.CategoryProductId;
      obj.Name = this.editedItem.Name;     
      this.IsSnackBar = true;
    ...
    delete(id) {
      ...
      axios({
        method: "delete",
        url:
          this.$urlApplication + "CategoryProduct/Delete?companyId=1&id=" + pId
      })
        .then(res => {
          this.IsSnackBar = false;
          this.tableLoad();
    ...
    clear() {
      this.editedItem.CategoryProductName = "";
    ...
    deleteItem(item) {
      ...
      confirm("Are you sure you want to delete this item?") &&
        this.delete(item.CategoryProductId);
    },
    dialogOpen() {
      if (this.editedItem.CategoryProductId == undefined) {
        //When click add
        this.editedItem.Active = true;
        this.clear();
      } else {
        //Edite Record
        //this.changeCategoryProducts(this.selectCategoryProducts);
      }
    },
    close() {
      if (!this.dialog) {

  ...
  <v-layout row wrap class="pa-2">
    <v-flex lg12 class="mt-12">
      <v-card class="black--text elevation-5">
        <v-layout>
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>Teritory</v-card-title>
            <v-card-title>
            ...
            <v-layout lg12>
              <v-flex lg12 class="pl-1 pr-1">
                <table class="csmTable">
                  <thead>
                    <tr>
                      <th v-on:click="sortTable('RegionName',$event)">
                        Region
                        <v-icon style="font-size:15px;text-align: right;">keyboard_arrow_up</v-icon>
                      </th>
                      <th v-on:click="sortTable('ZoneName',$event)">
                        Zone
                        <v-icon style="font-size:15px;text-align: right;">keyboard_arrow_up</v-icon>
                      </th>
                  ...
                  <tbody>
                    <tr v-for="row in rows">
                      <td class="text-xs-left">{{row.RegionName}}</td>
                      <td class="text-xs-left">{{row.ZoneName}}</td>
                      ...
                      <td class="justify-center layout px-0">
                        <v-btn
                          ...
                          :to="urlRoute+'Edit?id=' +row.SaleGeographyTerritoryId"
                        >

...
export default {
  ...
  methods: {
    async fetchDataAnalyst() {
      if (this.isLoadingAnalyst) {
        alert("It cannot be processed until the first request completes");
      } else {
    ...
    async fetchReportZoneBySupportGroupNR() {
      if (this.isLoadingSupportGroup) {
        ...
        if (difference < 0) {
          ...
          this.isLoadingSupportGroup = true;
          ...
          const res = await TicketService.getReportZoneBySupportGroupNR({
            ...
            .then(res => {
              ...
              this.isLoadingSupportGroup = false;
            })
            .catch(error => {
              this.isLoadingSupportGroup = false;
            });

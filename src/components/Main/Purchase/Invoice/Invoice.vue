  ...
  <v-layout row wrap class="pa-2">
    <v-flex lg12 class="mt-12">
      <v-card class="black--text elevation-5">
        <v-layout>
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>Purchase Invoice</v-card-title>
            <v-card-title>
              ...
              <v-text-field
                ...
                hide-details
              ></v-text-field>
              <v-btn slot="activator" color="primary" dark class="mb-2" to="/PurchaseInvoiceEdit?id=0">Add</v-btn>
            </v-card-title>
            <v-layout lg12>
                ...
                <table class="csmTable">
                  <thead>
                    <tr>
                      ...
                      <th v-on:click="sortTable('PurchasePersonName',$event)">
                        Purchase Person
                        <v-icon style="font-size:15px">keyboard_arrow_up</v-icon>
                      </th>
                  ...
                  <tbody>
                    <tr v-for="row in rows">
                      ...
                      <td class="text-xs-left">{{row.PurchasePersonName}}</td>
                      ...
                      <td class="justify-center layout px-0">
                        <v-btn
                          ...
                          :to="'/PurchaseInvoiceEdit?id=' +row.PurchaseInvoiceId"
                        >
...
export default {
  ...
  methods: {
    ...
    async tableLoad() {
      ...
      axios({
        method: "put",
        url: this.$urlApplication + "PurchaseInvoice/GetPurchaseInvoice",
        data: pagedResult
      })
        .then(res => {
    ...
    delete(id) {
      ...
      axios({
        method: "delete",
        url: this.$urlApplication + "PurchaseInvoice/Delete?companyId=1&id=" + pId
      })
        .then(res => {
    ...
    deleteItem(item) {
      confirm("Are you sure you want to delete this item?") &&
        this.delete(item.PurchaseInvoiceId);
    }
  }
};
</script>

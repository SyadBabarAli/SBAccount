        ...
        <v-layout>
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>
              Purchase Invoice - {{this.editedItem.SaleInvoiceId}} - {{this.editedItem.StatusName}}
              <v-toolbar-items>
                ...
                <v-btn flat color="colorLight" to="/PurchaseInvoice">close</v-btn>
              </v-toolbar-items>
            </v-card-title>
            <v-card>
              <v-layout>
                <v-flex lg2 class="pa-1">
                  <auto-complete
                    ...
                    :isAsync="true"
                    :apiUrl="'PurchaseInvoice/GetCustomer?pSearch='"
...
import TableInline from "../../../../components/Main/Purchase/Invoice/TableInlineInvoice";
...
export default {
  ...
  methods: {
    onChildClick(obj) {
      if (obj != null) {
        this.editedItem.PurchaseInvoiceDetail = obj.tableRow;
    ...
    async tableLoadDetail(id) {
      let response = await axios.get(
        ...
        this.$urlApplication + "PurchaseInvoice/" + id
      );
      if (id == 0) {
      ...
      this.listOfRecordDetails = data.PurchaseInvoiceDetailModel;
      this.editedItem.PurchaseInvoiceDetail = data.PurchaseInvoiceDetailModel;

      this.editedItem.GroupBranchId = data.GroupBranchId;
      this.editedItem.itemsBranch = data.groupBranchModel;
      
      this.editedItem.CustomerName = data.CustomerName;
       this.editedItem.CustomerId =data.CustomerId;
    ...
    post() {
      ...
      axios({
        method: "post",
        url: this.$urlApplication + "PurchaseInvoice/Post",
    ...
    clear() {
      ...
      this.editedItem.PurchaseInvoiceDetail = [];
    ...
    async getProposedNumber() {
      const res = await axios
        .get(this.$urlApplication + "PurchaseInvoice/GetProposedNumber")
        .then(res => {

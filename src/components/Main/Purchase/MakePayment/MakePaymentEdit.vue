        ...
        <v-layout>
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>
              Make Payment - {{this.editedItem.SaleRecurringInvoiceId}} - {{this.editedItem.StatusName}}
              <v-toolbar-items>
                ...
                <v-btn flat color="colorLight" to="/MakePayment">close</v-btn>
              </v-toolbar-items>
            </v-card-title>
            <v-card>
              <v-layout>
                <v-flex lg4 class="pa-1">
                  <auto-complete
                    ...
                    :isAsync="true"
                    :apiUrl="'PurchaseMakePayment/GetCustomer?pSearch='"
                ...
                <v-flex lg3 class="pa-1">
                  <v-text-field
                    ...
                    :append-icon="'today'"
                    type="date"
                    :value="MakePaymentDate && MakePaymentDate.toISOString().split('T')[0]"
                  ></v-text-field>
                </v-flex>
                <v-flex lg2 class="pa-1">
                  <v-text-field
...
import TableInline from "../../../../components/control/Grid/TableInlineReceiveMoney";
...
export default {
  ...
  data() {
    return {
      ...
      editedItem: {
        ...
        selectSettingFrequency: null,
        itemsSettingFrequency: [],
        SettingFrequencyId: 0,
  ...
  methods: {
    onChildClick(obj) {
      if (obj != null) {
        this.editedItem.PurchaseMakePaymentDetail = obj.tableRow;
        //this.editedItem.GrossAmount = obj.subTotal;
        this.editedItem.NetAmount = obj.subTotal;
        debugger;
      }
    },
    ...
    async tableLoadDetail(id) {
      let response = await axios.get(
        this.$urlApplication + "PurchaseMakePayment/" + id
      );
      if (id == 0) {
      ...
      this.listOfRecordDetails = data.PurchaseMakePaymentDetail;
      this.editedItem.PurchaseMakePaymentDetail =
        data.PurchaseMakePaymentDetail;
      ...
      // this.editedItem.selectSettingFrequency = this.refillSelect2(
      //   this.editedItem.SettingFrequencyId,
      //   data.settingFrequencyModel,
      //   "SettingFrequencyId"
      // );
    ...
    post() {
      ...
      this.editedItem.MakePaymentDate = this.MakePaymentDate;
      this.IsSnackBar = true;

      axios({
        method: "post",
        url: this.$urlApplication + "PurchaseMakePayment/Post",
    ...
    clear() {
      ...
      this.editedItem.MakePaymentDate = new Date();
    ...
    async getProposedNumber() {
      const res = await axios
        .get(this.$urlApplication + "PurchaseMakePayment/GetProposedNumber")
        .then(res => {

  ...
  <v-layout row wrap class="pa-2">
    <v-flex lg12 class="mt-12">
      <v-card class="black--text elevation-5">
        <v-layout> 
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>Make Payment</v-card-title>
            <v-card-title>
                ...
                <table class="csmTable">
                  <thead>
                    <tr>
                      ...
                      <th v-on:click="sortTable('MakePaymentDate')">Date</th>
                  ...
                  <tbody>
                    <tr v-for="row in rows">
                      ...
                      <td class="text-xs-left">{{row.MakePaymentDate | momentConvertDate }}</td>
                      ...
                      <td class="justify-center layout px-0">
                        <v-btn
                          ...
                          :to="'/MakePaymentEdit?id=' +row.PurchaseMakePaymentId"
                        >
...
export default {
  ...
  methods: {
    ...
    async tableLoad() {
      ...
      axios({
        method: "put",
        url: this.$urlApplication + "PurchaseMakePayment/GetPurchaseMakePayment",
        data: pagedResult
      })
        .then(res => {
    ...
    delete(id) {
      ...
      axios({
        method: "delete",
        url:
          this.$urlApplication + "PurchaseMakePayment/Delete?companyId=1&id=" + pId
      })
        .then(res => {
    ...
    deleteItem(item) {
      confirm("Are you sure you want to delete this item?") &&
        this.delete(item.PurchaseMakePaymentId);
    }
  }
};
</script>

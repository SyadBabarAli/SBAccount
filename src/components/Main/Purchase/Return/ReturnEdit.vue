        ...
        <v-layout>
          <v-flex lg12>
            <v-card-title class="headline grey lighten-3 pa-1 ma-0" primary-title>
              Purchase Return - {{this.editedItem.PurchaseReturnId}} - {{this.editedItem.StatusName}}
              <v-toolbar-items>
                ...
                <v-btn flat color="colorLight" to="/PurchaseReturn">close</v-btn>
              </v-toolbar-items>
            </v-card-title>
            <v-card>
              <!-- <v-layout>
              ...
              <v-layout>
                <v-flex lg2 class="pa-1">
                  <auto-complete
                    ...
                    :isAsync="true"
                    :apiUrl="'PurchaseReturn/GetCustomer?pSearch='"
                ...
                <v-flex class="pa-1">
                  <v-text-field
                    ...
                    :append-icon="'today'"
                    type="date"
                    :value="PurchaseReturnDate && PurchaseReturnDate.toISOString().split('T')[0]"
                  ></v-text-field>
                </v-flex>
                <v-flex class="pa-1">
                  <v-text-field
...
export default {
  ...
  methods: {
    onChildClick(obj) {
      if (obj != null) {
        this.editedItem.PurchaseReturnDetail = obj.tableRow;
    ...
    async tableLoadDetail(id) {
      let response = await axios.get(this.$urlApplication + "PurchaseReturn/" + id);
      if (id == 0) {
      ...
      this.listOfRecordDetails = data.PurchaseReturnDetailModel;
      this.editedItem.PurchaseReturnDetail =
        data.PurchaseReturnDetailModel;
    ...
    post() {
      ...
      this.editedItem.PurchaseReturnDate = this.PurchaseReturnDate;
      this.IsSnackBar = true;

      axios({
        method: "post",
        url: this.$urlApplication + "PurchaseReturn/Post",
    ...
    put() {
      ...
      axios({
        ...
        url:
          this.$urlApplication +
          "PurchaseReturn/Put?id=" +
          this.editedItem.PurchaseReturnId,
        data: this.editedItem
      })
        .then(res => {
          this.$router.push("/PurchaseReturn");
    ...
    clear() {
      ...
      this.AutoCompelete = "";
      this.editedItem.PurchaseReturnDate = new Date();
    ...
    async getProposedNumber() {
      const res = await axios
        .get(this.$urlApplication + "PurchaseReturn/GetProposedNumber")
        .then(res => {
